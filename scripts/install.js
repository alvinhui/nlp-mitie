...
var fs = require('fs'),
  ...
  path = require('path'),
  mitie = require('../lib/extensions'),
  request = require('request'),
  log = require('npmlog'),
  downloadOptions = require('./utils/download_options');
...
function checkAndDownloadBinary() {
  if (process.env.SKIP_MITIE_BINARY_DOWNLOAD_FOR_CI) {
    console.log('Skipping downloading binaries on CI builds');
    return;
  }

  var binaryPath = mitie.getBinaryPath(),
    hasBinary = mitie.hasBinary(binaryPath),
    cachedBinary = mitie.getCachedBinary(),
    cachePath = mitie.getBinaryCachePath(),
    binaryUrl = mitie.getBinaryUrl(),
    binaryName = mitie.getBinaryName();

  if (hasBinary) {
    console.log('nlp-mitie build', 'Binary found at', binaryPath);
    return;
  }
  ...
  download(binaryUrl, binaryPath, function(err) {
    if (err) {
      console.error(err);
      return;
    }
    ...
    cachedBinary = path.join(cachePath, binaryName);
