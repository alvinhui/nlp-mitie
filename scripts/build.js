...
function afterBuild(options) {
  var install = mitie.getBinaryPath();
...
function parseArgs(args) {
  ...
  options.args = args.filter(function(arg) {
    if (arg === '-f' || arg === '--force') {
      ...
      options.debug = true;
    } else if (arg.substring(0, 13) === '--libmitie_ext' && arg.substring(14) !== 'no') {
      options.libmitieExt = true;
    }
...
function testBinary(options) {
  if (options.force || process.env.MITIE_FORCE_BUILD) {
    return build(options);
  }

  if (!mitie.hasBinary(mitie.getBinaryPath())) {
    return build(options);
  }

  console.log('Binary found at', mitie.getBinaryPath());
  console.log('Testing binary');

  try {
    const {NerTrainingInstance} = require('../');
    const sample  = new NerTrainingInstance(['My', 'name']);
